...
colnames(mapmeans[4]) <- "OLD.NEIGHBORHOOD"
colnames(mapmeans[23]) <- "NEIGHBORHOOD" # This is the right way of renaming.
...
# mapcoord <- subset(mapcoord, select = -APART.MENT.NUMBER)
# mapcoord <- subset(mapcoord, select = -EASE.MENT)
